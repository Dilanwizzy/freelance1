...
export function createHeadersFromTokens(tokens: Partial<Tokens>): Headers {
	...
	const { accessToken, refreshToken, antiCsrfToken, frontToken } = tokens;
	
	if (!accessToken)
		headers.append(headerName, serialize(authCookieNames.access, '', deleteCookieSettings));
	else {
		headers.append(
			headerName,
	...
	if (!refreshToken)
		headers.append(headerName, serialize(authCookieNames.refresh, '', deleteRefreshSettings));
	else
		headers.append(
			headerName,
			serialize(authCookieNames.refresh, refreshToken, createCookieSettings('refresh'))
		);

	if (!antiCsrfToken)
		headers.append(headerName, serialize(authCookieNames.csrf, '', deleteCookieSettings));
	else
		headers.append(
			headerName,
	...
	if (!frontToken)
		headers.append(headerName, serialize(authCookieNames.front, '', deleteCookieSettings));
	else
		headers.append(
			headerName,
			serialize(authCookieNames.front, frontToken, createCookieSettings())
		);
	return headers;
}
